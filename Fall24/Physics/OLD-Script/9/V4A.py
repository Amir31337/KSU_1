...
Fold 1: MSE = 0.95, Mean Relative Error = 1.05
Fold 2: MSE = 2.37, Mean Relative Error = 0.82
Fold 3: MSE = 0.96, Mean Relative Error = 1.07
Fold 4: MSE = 6.26, Mean Relative Error = 2.41
Fold 5: MSE = 0.91, Mean Relative Error = 1.26
Average MSE: 2.29 ± 2.06
Average Mean Relative Error: 1.32 ± 0.56
========================================

Mean Squared Error on Test Set (Original Scale): 0.9406
Mean Relative Error on Test Set: 0.8411
'''
import numpy as np
import pandas as pd
...
# Plot first set of epochs
plt.figure(figsize=(10, 5))
plt.plot(epochs[:PLOT_FIRST_EPOCHS], train_losses[:PLOT_FIRST_EPOCHS], label='Training Loss')
plt.plot(epochs[:PLOT_FIRST_EPOCHS], val_losses[:PLOT_FIRST_EPOCHS], label='Validation Loss')
plt.title(f'Loss Curve for First {PLOT_FIRST_EPOCHS} Epochs')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
...
# Plot remaining epochs, if any
if NUM_EPOCHS > PLOT_FIRST_EPOCHS:
    plt.figure(figsize=(10, 5))
    plt.plot(epochs[PLOT_FIRST_EPOCHS:], train_losses[PLOT_FIRST_EPOCHS:], label='Training Loss')
    plt.plot(epochs[PLOT_FIRST_EPOCHS:], val_losses[PLOT_FIRST_EPOCHS:], label='Validation Loss')
    plt.title(f'Loss Curve for Epochs {PLOT_FIRST_EPOCHS + 1} to {NUM_EPOCHS}')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True)
...
with torch.no_grad():
    for x_batch, y_batch in test_loader:
        ...
        z = torch.randn_like(y_batch).to(DEVICE)  # Ensure z matches the input dimension
...
print(f'Mean Relative Error on Test Set: {mean_relative_error:.4f}')
...
# Optional: If you want to see relative error per dimension
...
# ----------------------------------------
# (Optional) Save Relative Errors to a CSV
# ----------------------------------------
