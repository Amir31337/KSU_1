...
momenta_normalized = momenta  # Assuming momenta do not require normalization

# Split data into train, validation, and test sets
# Ensure 70% train, 15% validation, 15% test
...
class CVAE(nn.Module):
    def __init__(self, input_dim, latent_dim, condition_dim, hidden_layers, dropout_rate=0.3):
        super(CVAE, self).__init__()
        print("Autoencoder Layers:", hidden_layers)
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(p=dropout_rate)
            # Uncomment the following lines to add Batch Normalization
...
cvae = CVAE(INPUT_DIM, LATENT_DIM, OUTPUT_DIM, AUTOENCODER_LAYERS, dropout_rate=DROPOUT_RATE).to(device)

# Optimizer with L2 regularization (weight_decay)
cvae_optimizer = optim.Adam(cvae.parameters(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)
...
cvae.load_state_dict(torch.load('cvae_weights_CINNV4.pth', map_location=device))
cvae.eval()
with torch.no_grad():
...
with torch.no_grad():
    cvae.eval()
    # Sample z from the training latent distribution
...
# Uncomment the following line if you wish to see MAE
