...
# Set hyperparameters (except L1_LAMBDA) based on the provided values
LATENT_DIM = 1024
EPOCHS = 65
BATCH_SIZE = 512
...
# Hyperparameter grid for L1_LAMBDA
L1_LAMBDA_VALUES = [1e-12, 1e-11, 1e-10, 1e-9, 1e-8, 1e-7, 1e-6, 1e-5, 1e-4]

# To store results
hyperparameter_results = []

# Iterate over each L1_LAMBDA
for L1_LAMBDA in L1_LAMBDA_VALUES:
    print(f"\nTraining with L1_LAMBDA = {L1_LAMBDA}")
    ...
    # Training loop with early stopping and L1 regularization
    cvae_losses = []
    val_losses = []
    ...
    for epoch in range(EPOCHS):
        ...
        train_loss_epoch /= batches_processed
        ...
        if val_batches_processed > 0:
            val_loss_epoch /= val_batches_processed
        ...
        if not np.isnan(val_loss_epoch) and val_loss_epoch < best_val_loss - MIN_DELTA:
            ...
            patience_counter = 0
            # Save the model
            model_path = f'best_cvae_L1_{L1_LAMBDA}.pth'
            torch.save(cvae.state_dict(), model_path)
            model_saved = True
        else:
            patience_counter += 1
            if patience_counter >= PATIENCE:
                print(f'Early stopping at epoch {epoch+1} for L1_LAMBDA = {L1_LAMBDA}')
                break

    # Check if a model was saved
    if not model_saved:
        print(f"No valid model was saved during training for L1_LAMBDA = {L1_LAMBDA}.")
        continue  # Skip evaluation if no model was saved
    ...
    with torch.no_grad():
        ...
        latent_stats_path = f'latent_stats_L1_{L1_LAMBDA}.pt'
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.title(f'Loss Curves (First 10 Epochs) - L1_LAMBDA={L1_LAMBDA}')
    plt.savefig(f'loss_curves_first_10_epochs_L1_{L1_LAMBDA}.png')
    plt.close()
    ...
    if len(cvae_losses) > 10:
        ...
        plt.ylabel('Loss')
        plt.legend()
        plt.title(f'Loss Curves (Epochs 11 onwards) - L1_LAMBDA={L1_LAMBDA}')
        plt.savefig(f'loss_curves_rest_epochs_L1_{L1_LAMBDA}.png')
        plt.close()
    else:
    ...
    print(f"Test MSE for L1_LAMBDA={L1_LAMBDA}: {mse}")
    print(f"Test MRE for L1_LAMBDA={L1_LAMBDA}: {mre}")

    # Save results for this L1_LAMBDA
    results = {
    ...
    # Optionally include L1_LAMBDA in hyperparameters
    # results['hyperparameters']['L1_LAMBDA'] = L1_LAMBDA

    hyperparameter_results.append(results)

    # Save individual results to a JSON file
    individual_result_path = f'results_L1_{L1_LAMBDA}.json'
    with open(individual_result_path, 'w') as f:
        json.dump(results, f)

# After all L1_LAMBDA runs, find the best one
if hyperparameter_results:
    # Convert to DataFrame for easier analysis
    results_df = pd.DataFrame(hyperparameter_results)
    # Find the row with the minimum MRE
    best_result = results_df.loc[results_df['mre'].idxmin()]
    best_L1_LAMBDA = best_result['L1_LAMBDA']
    best_mre = best_result['mre']
    best_mse = best_result['mse']

    print("\nBest L1_LAMBDA found:")
    print(f"L1_LAMBDA = {best_L1_LAMBDA}")
    print(f"Test MSE = {best_mse}")
    print(f"Test MRE = {best_mre}")

    # Save all results to a single JSON file
    with open('hyperparameter_search_results.json', 'w') as f:
        json.dump(hyperparameter_results, f)

    # Optionally, save the best model's hyperparameters and results
    best_model_info = best_result.to_dict()
