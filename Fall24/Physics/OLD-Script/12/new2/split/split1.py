...
# Define seeds and split ratios
seeds = [42, 100, 1234, 2024, 999]
splits = [
    (0.7, 0.15, 0.15),   # 70% train, 15% val, 15% test
    (0.6, 0.2, 0.2),     # 60% train, 20% val, 20% test
    (0.8, 0.1, 0.1),     # 80% train, 10% val, 10% test
    (0.75, 0.125, 0.125),# 75% train, 12.5% val, 12.5% test
    (0.65, 0.175, 0.175) # 65% train, 17.5% val, 17.5% test
]
...
# Iterate over each seed and split
for seed in seeds:
    for split_idx, (train_ratio, val_ratio, test_ratio) in enumerate(splits, 1):
        print(f"\n=== Running Seed: {seed}, Split: {split_idx} ({train_ratio*100}% train, {val_ratio*100}% val, {test_ratio*100}% test) ===")
        ...
        try:
            train_position, temp_position, train_momenta, temp_momenta = train_test_split(
                position, momenta, test_size=(1 - train_ratio), random_state=seed, shuffle=True
            )
            val_size = val_ratio / (val_ratio + test_ratio)
            val_position, test_position, val_momenta, test_momenta = train_test_split(
                temp_position, temp_momenta, test_size=(1 - val_size), random_state=seed, shuffle=True
            )
        ...
        for epoch in range(EPOCHS):
            ...
            if val_loss < best_val_loss - MIN_DELTA:
                ...
                patience_counter = 0
                # Save the model
                model_path = f'best_model_seed{seed}_split{split_idx}.pth'
        ...
        # Load the best model
        best_model_path = f'best_model_seed{seed}_split{split_idx}.pth'
        if os.path.exists(best_model_path):
            model.load_state_dict(torch.load(best_model_path, map_location=device))
        else:
            print(f"Best model not found for seed {seed}, split {split_idx}. Using current model.")
        ...
        with torch.no_grad():
            ...
            # Save latent stats
            latent_stats_path = f'latent_stats_seed{seed}_split{split_idx}.pt'
        ...
        print(f"Test MRE for Seed {seed}, Split {split_idx}: {mre:.6f}")
        ...
        # Save the MRE to a JSON file for this run
        run_results = {
            'seed': seed,
            'split': split_idx,
            'mre': float(mre)
        }
        run_results_path = f'results_seed{seed}_split{split_idx}.json'
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.title(f'Train & Val Loss - First 10 Epochs (Seed {seed}, Split {split_idx})')
        plt.legend()
        plt.grid(True)
        plt.savefig(f'first10_seed{seed}_split{split_idx}.png')
        plt.close()
        ...
        if len(train_losses) > 10:
            ...
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            plt.title(f'Train & Val Loss - Remaining Epochs (Seed {seed}, Split {split_idx})')
            plt.legend()
            plt.grid(True)
            plt.savefig(f'rest_epochs_seed{seed}_split{split_idx}.png')
            plt.close()

        # Model Summaries
        print(f"\n--- Model Summary using torchsummary for Seed {seed}, Split {split_idx} ---")
        try:
            torchsummary.summary(model, input_size=[(INPUT_DIM,), (OUTPUT_DIM,)])
        except Exception as e:
            print(f"torchsummary failed: {e}")

        print(f"\n--- Model Summary using torchinfo for Seed {seed}, Split {split_idx} ---")
        try:
...
for res in results:
    print(f"Seed: {res['seed']}, Split: {res['split']} | MRE: {res['mre']:.6f}")
