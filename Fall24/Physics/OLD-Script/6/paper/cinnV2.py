...
CONFIG = {
    ...
    'momenta_dim': 9,
    'num_epochs': 100,
    'data_file': '/home/g/ghanaatian/MYFILES/FALL24/Physics/Fifth/cei_traning_orient_1.csv',
...
def train(model, train_loader, optimizer, device, kl_weight):
    model.train()
    train_loss = 0
...
def evaluate(model, test_loader, latent_representations, device):
    model.eval()
    test_mse = 0
    mse_loss = nn.MSELoss()
    latent_idx = 0
    ...
    with torch.no_grad():
        for x, y in test_loader:
            ...
            z = latent_representations[latent_idx:latent_idx+batch_size].to(device)
            latent_idx += batch_size
            
            c = model.conditional_net(y)
            x_recon = model.decode(z, c)
            
            test_mse += mse_loss(x_recon, x.to(device)).item()
...
def cross_validate(config):
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X)):
        ...
        for epoch in range(config['num_epochs']):
            train_loss, latent_representations = train(model, train_loader, optimizer, device, config['kl_weight'])

        val_mse = evaluate(model, val_loader, latent_representations, device)
...
def objective(trial):
    config = {
        ...
        'kl_weight': trial.suggest_loguniform('kl_weight', 1e-4, 1),
