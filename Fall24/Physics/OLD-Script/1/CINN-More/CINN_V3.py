...
def load_data(filepath):
    ...
    HERE
    '''
    mc = 21894.713607956142
    mo = 29164.39289099079
    ms = 58441.80486812706

        # Create the condition variables
    condition_vars = np.zeros_like(features)

    # Apply the multiplications
    condition_vars[:, 0] = features[:, 0] * mc  # pcx * mc
    condition_vars[:, 1] = features[:, 1] * mc  # pcy * mc
    condition_vars[:, 2] = features[:, 2] * mc  # pcz * mc
    condition_vars[:, 3] = features[:, 3] * mo  # pox * mo
    condition_vars[:, 4] = features[:, 4] * mo  # poy * mo
    condition_vars[:, 5] = features[:, 5] * mo  # poz * mo
    condition_vars[:, 6] = features[:, 6] * ms  # psx * ms
    condition_vars[:, 7] = features[:, 7] * ms  # psy * ms
    condition_vars[:, 8] = features[:, 8] * ms  # psz * ms
