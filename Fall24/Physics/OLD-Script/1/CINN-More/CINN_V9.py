...
def load_data(filepath):
    ...
    # Calculate angles, clipping the input to arccos to avoid NaN
    theta_co = np.arccos(np.clip(p_c_dot_p_o / (p_c_mag * p_o_mag), -1.0, 1.0))
    theta_cs = np.arccos(np.clip(p_c_dot_p_s / (p_c_mag * p_s_mag), -1.0, 1.0))
    theta_os = np.arccos(np.clip(p_o_dot_p_s / (p_o_mag * p_s_mag), -1.0, 1.0))
