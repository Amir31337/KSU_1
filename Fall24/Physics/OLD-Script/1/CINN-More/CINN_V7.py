...
def load_data(filepath):
    ...
    # Calculate the magnitude of momentum for each particle
    p_c_mag = np.sqrt(features[:, 0]**2 + features[:, 1]**2 + features[:, 2]**2)
    p_o_mag = np.sqrt(features[:, 3]**2 + features[:, 4]**2 + features[:, 5]**2)
    p_s_mag = np.sqrt(features[:, 6]**2 + features[:, 7]**2 + features[:, 8]**2)

    # Calculate pairwise momentum differences
    delta_p_co = p_c_mag - p_o_mag
    delta_p_cs = p_c_mag - p_s_mag
    delta_p_os = p_o_mag - p_s_mag

    # Stack the differences to create the conditions array
    conditions = np.column_stack((delta_p_co, delta_p_cs, delta_p_os))
