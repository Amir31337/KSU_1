...
def load_data(filepath):
    ...
    p_c_mag = np.sqrt(np.sum(features[:, 0:3]**2, axis=1))
    p_o_mag = np.sqrt(np.sum(features[:, 3:6]**2, axis=1))
    p_s_mag = np.sqrt(np.sum(features[:, 6:9]**2, axis=1))

    # Stack the magnitudes to create the conditions array
    conditions = np.column_stack((p_c_mag, p_o_mag, p_s_mag))
