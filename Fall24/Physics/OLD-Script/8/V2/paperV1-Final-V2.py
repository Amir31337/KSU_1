...
def train_and_evaluate():
    ...
    input_dim = 9
    condition_dim = 9
    cinn_model = create_cinn_model(input_dim, condition_dim, hidden_features=256, num_blocks=6)
    ...
    for epoch in tqdm(range(1, num_epochs + 1), desc='Training'):
        ...
        val_loss = validate(cinn_model, val_loader)
    ...
    plt.close()
    print('Training and validation loss plot saved as loss_plot_first_40_epochs.png')
    ...
    # Down-selection: Select top 3 samples that best match the desired momenta
    top_k = 3
    ...
    X_test_original = position.loc[X_test.index].values
    predicted_X_single_original = scaler_position.inverse_transform(predicted_X_single)
