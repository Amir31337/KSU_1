...
# Fixed dimensions based on the data
POSITION_DIM = 9
MOMENTA_DIM = 9
...
def create_cinn(subnet_hidden_dim, num_coupling_blocks, coupling_clamp):
...
def k_fold_cross_validation(data, k_folds, hyperparams):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(position)):
        ...
        model = create_cinn(hyperparams['subnet_hidden_dim'], 
                            hyperparams['num_coupling_blocks'], 
                            hyperparams['coupling_clamp'])
