...
config = {
    ...
    'output_size': 9,
    'condition_size': 9  # Adding condition size
}

# Define the model architecture with conditioning
class cINN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, condition_size):
        super(cINN, self).__init__()
        self.encoder = nn.Sequential(
            nn.Linear(input_size + condition_size, hidden_size),  # Concatenate condition
            nn.ReLU(),
        ...
        self.decoder = nn.Sequential(
            nn.Linear(output_size + condition_size, hidden_size),  # Concatenate condition
            nn.ReLU(),
            nn.Linear(hidden_size, hidden_size),
            nn.ReLU(),
            nn.Linear(hidden_size, output_size),
    ...
    def forward(self, x, condition):
        x = torch.cat((x, condition), dim=1)  # Concatenate input with condition
...
def train(model, device, train_loader, criterion, optimizer):
    ...
    for data, target, condition in train_loader:
        data, target, condition = data.to(device), target.to(device), condition.to(device)  # Move data to GPU
        optimizer.zero_grad()
...
def test(model, device, test_loader, scaler_targets):
    ...
    with torch.no_grad():
        for data, target, condition in test_loader:
            data, target, condition = data.to(device), target.to(device), condition.to(device)  # Move data to GPU
            output = model(data, condition)
            output = output.cpu()  # Move output back to CPU for numpy conversion
...
def main():
    ...
    # Prepare DataLoader
    dataset = TensorDataset(features_tensor, targets_tensor, targets_tensor)  # Using targets as condition
    ...
    # Test the model
    model.load_state_dict(torch.load('cinn_model.pth', weights_only=True))
