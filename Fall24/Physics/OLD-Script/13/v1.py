...
# Define the optimizer with L2 regularization through weight decay
...
# Save the MRE to a JSON file, ensuring it's a native Python float
results = {
...
# --- Added Function to Print 5 Random Test Examples ---
def print_random_test_examples(test_positions, test_momenta, test_predictions, num_examples=5):
    """
    Prints 5 random examples from the test set with their original positions,
    momenta, and reconstructed positions. Each value is formatted to two decimal places.
    
    Parameters:
    - test_positions (np.ndarray): Original test positions in original scale.
    - test_momenta (np.ndarray): Original test momenta in original scale.
    - test_predictions (np.ndarray): Reconstructed positions in original scale.
    - num_examples (int): Number of random examples to print.
    """
    ...
    for idx in random_indices:
        original_position = test_positions[idx]
        original_momenta = test_momenta[idx]
        reconstructed_position = test_predictions[idx]
        
        # Format the outputs with two decimal places
        formatted_position = ' '.join([f"{val:.2f}" for val in original_position])
        formatted_momenta = ' '.join([f"{val:.2f}" for val in original_momenta])
        formatted_reconstructed = ' '.join([f"{val:.2f}" for val in reconstructed_position])
...
# Call the function to print 5 random test examples
print_random_test_examples(test_position_inv, test_momenta.cpu().numpy(), test_predictions_inv, num_examples=5)
