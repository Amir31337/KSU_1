...
SAVE_DIR_BASE = '/home/g/ghanaatian/MYFILES/FALL24/Physics/CVAE/Physics-Nov21st/new'
...
def run_experiment(config):
    ...
    def cvae_loss_fn(recon_x, x, mu, logvar, model, training=True):
        # Mean Relative Error (MRE) as reconstruction loss
        recon_loss = torch.mean(torch.abs(x - recon_x) / (torch.abs(x) + 1e-8))
        ...
        if training:
            ...
            # Do not include l2_reg here to avoid double-counting
            pass
        else:
            # Do not include regularization terms in validation/test loss
            pass
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, (position_batch, momenta_batch) in enumerate(train_loader):
            ...
            with autocast_context:
                ...
                loss = cvae_loss_fn(recon_position, position_batch, mu, logvar, cvae, training=True)
        ...
        with torch.no_grad():
            for position_batch, momenta_batch in val_loader:
                ...
                with autocast_context:
                    recon_position, mu, logvar = cvae(position_batch, momenta_batch)
                    loss = cvae_loss_fn(recon_position, position_batch, mu, logvar, cvae, training=False)
